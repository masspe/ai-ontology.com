...
import { DEFAULT_PROVIDER_CONFIG, PROVIDER_CONFIG_STORAGE_KEY, } from "../types/providerConfig";
...
export function saveProviderConfig(cfg) {
    ...
    try {
        ...
        if (url)
            window.localStorage.setItem("ontology.apiBase", url);
        else
            window.localStorage.removeItem("ontology.apiBase");
        const tok = cfg.ontologyBearerToken.trim();
        if (tok)
            window.localStorage.setItem("ontology.apiToken", tok);
        else
...
export function apiKeyForProvider(cfg, provider) {
    switch (provider) {
        case "openai":
 ...
 * non-empty values.
 */
export function getActiveProviderRequestFields(cfg = loadProviderConfig(), override) {
    ...
    if (provider === "infomaniak") {
        if (cfg.infomaniakBaseUrl)
            out.base_url = cfg.infomaniakBaseUrl;
        if (cfg.infomaniakModel)
            out.model = cfg.infomaniakModel;
    }
    return out;
}
