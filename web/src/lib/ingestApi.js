...
export async function analyzeIngest(opts) {
    ...
    if (extra.api_key)
        form.append("api_key", extra.api_key);
    if (extra.base_url)
        form.append("base_url", extra.base_url);
    // Fill in a default model from cfg if caller didn't pass one.
    if (!opts.model && extra.model)
        form.append("model", extra.model);
    const headers = {};
    const tok = apiToken();
    if (tok)
    ...
    if (!res.ok) {
        ...
        throw new ApiError(`analyze failed: ${res.status} ${res.statusText}${suffix}`, res.status, body);
    }
