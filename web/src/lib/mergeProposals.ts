...
function isGraphRef(ref: string | undefined | null): boolean {
  if (!ref) return false;
  if (ref.startsWith("f") && /^f\d+\//.test(ref)) return false;
  return ref.includes(":");
}

/** Rewrite a single ref using a same-file map. Pass-through for graph refs
 * or refs not present in the map. */
function remap(ref: string, localMap: Map<string, string>): string {
  const mapped = localMap.get(ref);
  return mapped ?? ref;
}

/** Apply the global canonicalization map (used for cross-file dedupe). */
function canonicalize(ref: string, canonical: Map<string, string>): string {
...
export function mergeProposals(parts: ProposalPart[]): OntologyProposal {
  const concept_types: ProposalConceptType[] = [];
  const relation_types: ProposalRelationType[] = [];
  const concepts: ProposalConcept[] = [];
  const relations: ProposalRelation[] = [];
  const rules: ProposalRule[] = [];
  ...
  const rtByName = new Map<string, string>();
  const conceptByKey = new Map<string, string>(); // `${type}::${name}`
  const relationByKey = new Map<string, string>(); // `${type}::${src}::${tgt}` post-remap
  ...
  let firstLanguage: OntologyProposal["language"] = null;
  ...
  parts.forEach((part, idx) => {
    ...
    const collect = (refs: { client_ref: string }[]) => {
      for (const it of refs) localMap.set(it.client_ref, prefix + it.client_ref);
    };
    ...
    if (p.source?.encoding) encodings.add(p.source.encoding);
    if (p.language) {
      languages.add(p.language.code);
      if (!firstLanguage) firstLanguage = p.language;
    }
  });
  ...
  const fixRel = (r: ProposalRelation): ProposalRelation => ({
    ...r,
  ...
  const fixRule = (r: ProposalRule): ProposalRule => ({
    ...r,
    applies_to: r.applies_to?.map((x) =>
      isGraphRef(x) ? x : canonicalize(x, canonical),
    ),
  });
  const fixAction = (a: ProposalAction): ProposalAction => ({
    ...a,
