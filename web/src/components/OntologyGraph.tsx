...
interface Props {
  ...
  limit?: number;
  /** When false, hides the legend (caller renders its own). */
  showLegend?: boolean;
...
interface Layout {
  nodes: Array<{ id: number; label: string; type: string; x: number; y: number; w: number; h: number; color: number }>;
  edges: Array<{ from: number; to: number; label?: string }>;
  width: number;
  height: number;
  types: Array<{ name: string; color: number }>;
}
...
function layout(subgraph: Subgraph, w: number, h: number, limit: number): Layout {
  // assign a color slot per concept type
  const typeIndex = new Map<string, number>();
  for (const c of subgraph.concepts) {
    if (!typeIndex.has(c.concept_type)) typeIndex.set(c.concept_type, typeIndex.size % PALETTE.length);
  }
  ...
  for (const r of subgraph.relations) {
    if (r.source === concepts[0].id) hubNeighbors.add(r.target);
    if (r.target === concepts[0].id) hubNeighbors.add(r.source);
  }

  const inner: typeof concepts = [];
  const outer: typeof concepts = [];
  for (const c of concepts.slice(1)) {
    if (hubNeighbors.has(c.id) && inner.length < 6) inner.push(c);
    else outer.push(c);
  }
...
export default function OntologyGraph({
  ...
  const byId = useMemo(() => {
    ...
    for (const n of lay.nodes) m.set(n.id, n);
    return m;
  }, [lay.nodes]);
  ...
  return (
    <div className={`og-wrap${className ? " " + className : ""}`} style={{ minHeight: height }}>
      ...
      {showLegend && lay.types.length > 0 && (
        <ul className="og-legend">
          {lay.types.slice(0, 8).map((t) => {
            const p = PALETTE[t.color % PALETTE.length];
            return (
              <li key={t.name}>
                <span className="og-dot" style={{ background: p.stroke }} /> {t.name}
              </li>
            );
          })}
        </ul>
