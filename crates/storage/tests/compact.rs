...
async fn compact_preserves_symmetric_relation_count() {
    ...
    store.append(&LogRecord::ontology(g.ontology())).await.unwrap();

    let t1 = g.upsert_concept(Concept::new(Default::default(), "Topic", "RAG")).unwrap();
    let t2 = g.upsert_concept(Concept::new(Default::default(), "Topic", "ANN")).unwrap();
    let _ = g.add_relation(Relation::new(Default::default(), "related_to", t1, t2)).unwrap();
    store.append(&LogRecord::concept(g.get_concept(t1).unwrap())).await.unwrap();
    store.append(&LogRecord::concept(g.get_concept(t2).unwrap())).await.unwrap();
    ...
    assert_eq!(g2.relation_count(), pre_count,
...
async fn compact_truncates_log_and_replays_clean() {
    ...
    let paper = g.upsert_concept(Concept::new(Default::default(), "Paper", "P")).unwrap();
    ...
    let rid = g.add_relation(Relation::new(Default::default(), "authored", alice, paper)).unwrap();
    store.append(&LogRecord::relation(g.get_relation(rid).unwrap())).await.unwrap();
    ...
    store.append(&LogRecord::concept(g.get_concept(bob).unwrap())).await.unwrap();
    ...
    assert_eq!(g2.relation_count(), 1, "relation should not be double-applied");
