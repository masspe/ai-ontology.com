...
async fn deletes_replay_through_wal() {
    ...
    let rel_id = g.add_relation(Relation::new(Default::default(), "authored", alice, paper)).unwrap();
    store.append(&LogRecord::relation(g.get_relation(rel_id).unwrap())).await.unwrap();

    // Delete Alice and journal the cascade.
    let removed = g.remove_concept(alice).unwrap();
    store.append(&LogRecord::delete_concept(alice)).await.unwrap();
    for r in &removed {
